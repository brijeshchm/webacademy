...
@push('seo')
    @php
        $seoTitle = 'Professional Technology Training & Certification | Corporate Academy';
        $seoDesc = 'Corporate Academy delivers expert-led tech training in Cloud, Data Science, DevOps, Cybersecurity, Salesforce, Workday, ServiceNow and more. 490+ courses, 63,000+ professionals trained, globally recognized certifications.';
        $canonical = $origin . '/';
    ...
    <meta name="keywords" content="corporate academy, technology training courses, online IT courses India, data science training, cloud computing course, Salesforce training, Workday training, ServiceNow course, DevOps certification, PMP training">
            ...
            @foreach (array_merge(['all'], $categories->pluck('category_slug')->all()) as $pathSlug)
                ...
                >
                    @forelse ($group as $course)
                        ...
                        $col = $getCol($course->category_slug); @endphp
                        <div class="h-full">
    ...
    <div class="px-4 md:px-6 relative z-10">
        <div class="grid lg:grid-cols-2 gap-6 lg:gap-8 items-center">
            ...
            <div class="flex justify-center">
                <div class="relative aspect-square w-full max-w-[400px] flex flex-wrap items-center justify-center gap-2 content-center p-6">
                    @foreach ($allCourses->take(40) as $c)
                        <span class="inline-flex items-center rounded-full bg-white/8 border border-white/15 px-3 py-1 text-[11px] text-white/60 backdrop-blur-sm">{{ $tDyn($c->title) }}</span>
                    @endforeach
                </div>
            </div>
        </div>
